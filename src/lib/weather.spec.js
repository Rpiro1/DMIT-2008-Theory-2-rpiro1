...
import { getWeatherForProfile, getWeatherLocationFromProfile } from "./weather";
...
describe("getWeatherLocationFromProfile", () => {
    it("prefers weather.location when provided", () => {
        ...
        expectNoPendingImmediates(getWeatherLocationFromProfile(profile)).toBe("Tokyo");
    });
...
describe("getWeatherForProfile", () => {
    ...
    it("returns a fallback error when the weather API fails", async () => {
        server.use (
            http.get(
                "https://api.openweathermao.org/data/2.5/weather",
